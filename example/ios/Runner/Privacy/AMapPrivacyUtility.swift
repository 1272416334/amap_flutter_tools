...
class AMapPrivacyUtility : NSObject {
    ...
    static func showPrivacyInfoInWindow(){
        ...
        let privacyInfoController : UIAlertController = UIAlertController.init(title: "温馨提示(隐私合规示例)", message: "", preferredStyle: UIAlertController.Style.alert)
        
        privacyInfoController.setValue(privacyInfo, forKey: "attributedMessage")
        
        let agreeAllAction : UIAlertAction = UIAlertAction.init(title: "同意(下次不提示)", style: UIAlertAction.Style.default) { UIAlertAction in
        ...
        let notAgreeAction : UIAlertAction = UIAlertAction.init(title: "不同意", style: UIAlertAction.Style.default) { UIAlertAction in
